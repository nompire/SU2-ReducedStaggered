...
#define CMUL_SUM(a,b,c) { (c).real += (a).real* (b).real - (a).imag*(b).imag; \
          (c).imag += (a).real*(b).imag + (a).imag*(b).real; }
               ...
               /* c += s*a*b */
#define CMULREAL_SUM(a,b,s,c) { (c).real += (s) * (a).real * (b).real  -  (s) * (a).imag * (b).imag; \
          (c).imag += (s) * (a).real * (b).imag  + (s) * (a).imag * (b).real ; }

              /*  c -= s*a*b  */
#define CMULREAL_DIF(a,b,s,c) { (c).real -= (a).real*(b).real * (s) - (a).imag*(b).imag* (s); \
          (c).imag -= (a).real*(b).imag * (s) + (a).imag*(b).real* (s); }
